...
// 9) Exibir a mensagem:
//  "O valor correspondente à casa das centenas da placa [placadoveiculo] é igual a: [obtendooalgarismonacasadascentenas]".
...
int main(void) {
    ...
    int placadoveiculo;
    int obtendooalgarismonacasadascentenas;
    ...
    // Para extrair somente a casa das centenas, use:

    obtendooalgarismonacasadascentenas = (placadoveiculo / 100) % 10;

    // Explicação:

    // placadoveiculo / 100 remove os dois últimos dígitos (deixa apenas os dois primeiros).

    // 4567 / 100 = 45

    // 45 % 10 extrai o último dígito dos dois restantes (a casa das centenas).

    // 45 % 10 = 5

    printf("O valor correspondente à casa das centenas da placa %04d é igual a: %d", placadoveiculo, obtendooalgarismonacasadascentenas);
